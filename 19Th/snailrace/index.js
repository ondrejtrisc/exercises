class Snail {
  constructor(name, speed, color) {
    ...
    nameElm.className = 'snail-racer__name';
    nameElm.textContent = this.name;
    this.element.appendChild(nameElm);
    this.timer = setInterval(() => {
      if (this.position < window.innerWidth - 170) {
...
document.addEventListener('DOMContentLoaded', () => {
  fetch('http://bootcamp.podlomar.org/api/snails/4')
    ...
    .then(json => {
      for (const snailObj of json) {
        const snail = new Snail(snailObj.name, snailObj.velocity * 10, snailObj.color);
      }
    });
});